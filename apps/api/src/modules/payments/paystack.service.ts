...
export class PaystackService {
  ...
  async initializePayment(organizationId: string, input: InitializePaystackPaymentDto) {
    ...
    const [paid] = await this.db
      ...
      .from(payment)
      .where(and(eq(payment.organizationId, organizationId), eq(payment.orderId, sale.id)));

    const paidKobo = Number(paid?.totalKobo ?? 0);
    const amountKobo = sale.totalKobo - paidKobo;
    if (amountKobo <= 0) throw new BadRequestException("Sale has no outstanding balance.");
  ...
  async handleWebhook(signature: string | undefined, rawBody: Buffer, event: unknown) {
    this.verifyWebhookSignature(signature, rawBody);

    if (!this.isChargeSuccessEvent(event)) {
  ...
  private async applyTransaction(reference: string, transaction: PaystackTransaction) {
    ...
    if (transaction.currency !== link.currency || transaction.amount !== link.amountKobo) {
    ...
    if (transaction.status !== "success") {
      await this.db
        .update(paymentLink)
        .set({ status: transaction.status, updatedAt: new Date() })
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (!currentLink) throw new BadRequestException("Payment link not found.");
      ...
      const [existingPayment] = await tx
        ...
        .where(
          and(
            eq(payment.organizationId, currentLink.organizationId),
      ...
      const [paid] = await tx
        ...
        .where(
          and(
            eq(payment.organizationId, currentLink.organizationId),
            eq(payment.orderId, currentLink.orderId),
          ),
        );
