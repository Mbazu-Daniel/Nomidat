  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {error ? <div className="mb-4 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}
        {loading ? <div className="rounded-2xl border border-orange-100 bg-white p-6 text-sm text-muted-foreground">Loading invoices…</div> : null}
        {!loading && rows.length === 0 ? (
          <div className="rounded-2xl border border-orange-100 bg-white p-10 text-center text-sm text-muted-foreground">
        ...
        {!loading && rows.length > 0 ? (
          <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white">
            <div className="divide-y divide-orange-100">
              {rows.map((invoice) => (
                <div key={invoice.id} className="flex flex-wrap items-center gap-4 px-4 py-4 sm:px-5">
